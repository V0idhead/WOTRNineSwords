...
{
    static class StrengthDrainingStrike
    {
        ...
        {
            Main.Logger.Info($"Configuring {nameof(StrengthDrainingStrike)}");

            var ability = AbilityConfigurator.New("StrengthDrainingStrikeAbility", "54CCDCE7-D344-4155-A771-CE9C15829DA0")
              .SetDisplayName(name)
              .SetDescription(desc)
              .SetIcon(icon)
              ...
              (
                ...
                .SavingThrow(Kingmaker.EntitySystem.Stats.SavingThrowType.Fortitude, customDC: new ContextValue { Value = 13 }, conditionalDCModifiers: Helpers.GetManeuverDCModifier(Kingmaker.UnitLogic.Mechanics.Properties.UnitProperty.StatBonusWisdom, ShadowPresence.ShadowHandFocusFactGuid),
                    onResult: ActionsBuilder.New().ConditionalSaved(
                        failed: ActionsBuilder.New().Add<MeleeAttackWithStatDamage>(mawsd => { mawsd.statType = Kingmaker.EntitySystem.Stats.StatType.Strength; mawsd.damageAmount = new Kingmaker.RuleSystem.DiceFormula(4, Kingmaker.RuleSystem.DiceType.One); }),
                        succeed: ActionsBuilder.New().Add<MeleeAttackWithStatDamage>(mawsd => { mawsd.statType = Kingmaker.EntitySystem.Stats.StatType.Strength; mawsd.damageAmount = new Kingmaker.RuleSystem.DiceFormula(2, Kingmaker.RuleSystem.DiceType.One); })
                    )
                )
              )
            ...
            var maneuver = FeatureConfigurator.New("StrengthDrainingStrike", Guid)
              .SetDisplayName(name)
              .SetDescription(desc)
              .SetIcon(icon)
